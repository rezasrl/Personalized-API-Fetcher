...
final class Block {
	...
	public static function register_blocks() {
		...
		$blocks = apply_filters( 'personalized_api_fetcher_gutenberg_blocks', array() );
	...
	public static function register_admin( $block = '', $args = array() ) {
		...
		wp_register_script(
			'personalized-api-fetcher-' . $block_name . '-blocks',
	...
	public static function register( $block = '', $args = array() ) {
		...
		// Set editor styles.
		$editor_style = 'personalized-api-fetcher-' . $block_name . '-blocks';
		if ( ! empty( $args['editor_style'] ) ) {
			$editor_style = $args['editor_style'];
		}

		register_block_type(
			'personalized-api-fetcher/' . $block_name,
			array(
				'style'         => $styles,
				'script'        => $script,
				'editor_style'  => $editor_style,
				'editor_script' => 'personalized-api-fetcher-' . $block_name . '-blocks',
			)
		);
	}
}
